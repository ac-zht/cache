...
func TestMaxCntCache_Set(t *testing.T) {
	testCase := []struct {
	...
	}{
		{
			...
			cache: func() *MaxCntCache {
				res := NewMaxCntCache(10, local_cache.NewBuildInMapCache(10), MaxCntCacheWithEvictedCallback())
				return res
			},
			key:     "key",
			val:     "value",
		...
		},
		{
			name: "set fail",
			cache: func() *MaxCntCache {
				res := NewMaxCntCache(2, local_cache.NewBuildInMapCache(2), MaxCntCacheWithEvictedCallback())
...
func TestMaxCntCache_LoadAndDelete(t *testing.T) {
	testCase := []struct {
		...
		},
		{
			...
			key:       "k2",
			wantError: fmt.Errorf("%w, key: %s", cache.ErrKeyNotFound, "k2"),
		},
	}
	for _, tc := range testCase {
		t.Run(tc.name, func(t *testing.T) {
